...
#example creating a data sample
df1<-data.frame(path=c('facebook>website>itempage','facebook','website>itempage'),conv=c(1,0,0),conv_null=c(0,1,1))
...
#adding dummy variables to plot the graph
df_dummy<-data.frame(channel_from=c('(start)', '(conversion)','(null)'),
                     channel_to=c('(start)','(conversion)','(null)'),
                     transition_probability=c(0,1,1))
#combine df_trans and df_dummy on the same table by row
df_trans<-rbind(df_trans, df_dummy)

#ordering channels
#factor function refers to categorical variables
...
#plotting the graph
plot(trans_matrix, edge.arrow.size = 0.35)



#part 2
#simulate a dataset that looks like real data of custome journeys assuming all paths were finished with purchase/ conversion

#simulating the real data
#set.seed takes an arbitrary integer argument to get reproducible random numbers
set.seed(354)
...
df2$date<-as.Date(df2$date, origin="2018-01-01")
...
#heuristic model not supported everywhere so we use manual calculation however for reference
